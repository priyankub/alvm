...
from .ALVMObject import ALVMObject, SExpType
        ...
        if op == 0:
            ...
            if hasattr(v, "__iter__"):
                ...
                for _ in v:
                    ...
                    # type
                    if type(_) != ALVMObject:
                        ops.append((0, None))  # convert
                continue

            raise ValueError("can't cast to ALVMObject: %s" % v)
        if op == 1:  # set left
            stack[target] = (ALVMObject(stack.pop()), stack[target][1])
            continue
        if op == 2:  # set right
            stack[target] = (stack[target][0], ALVMObject(stack.pop()))
            continue
        if op == 3:  # prepend list
            stack[target] = (ALVMObject(stack.pop()), ALVMObject(stack[target]))
            continue
