...
from .ALVMObject import ALVMObject
...
def default_unknown_op(op: bytes, args: ALVMObject) -> Tuple[int, ALVMObject]:
...
class OperatorDict(dict):
    ...
    def __call__(self, op: bytes, arguments: ALVMObject) -> Tuple[int, ALVMObject]:
        f = self.get(op)
        if f is None:
